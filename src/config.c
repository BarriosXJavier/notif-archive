...
static int set_defaults(config_t *cfg) {
    ...
    if (n < 0 || (size_t)n >= sizeof(cfg->archive_root) ||
        ...
        return -1;
    if (!session_type ||
        (strcmp(session_type, "x11") != 0 && strcmp(session_type, "wayland") != 0))
        session_type = "x11";
    if (copy_string(cfg->session_type, sizeof(cfg->session_type), session_type) < 0)
        return -1;
...
int config_load(const char *path, config_t *out) {
    ...
    if (set_defaults(out) < 0) {
        log_msg(LOG_ERROR, "config: HOME default path is too long or invalid UTF-8");
        return -1;
    }
        ...
        *eq = '\0';
        {
            ...
            if (strcmp(key, "archive_root") == 0) {
                if (*value == '\0' || !valid_utf8(value) ||
                    expand_home(value, out->archive_root,
                                sizeof(out->archive_root)) < 0) {
                    log_msg(LOG_ERROR,
                ...
                if (strcmp(value, "x11") != 0 && strcmp(value, "wayland") != 0) {
                    log_msg(LOG_ERROR,
                            ...
                            line_number);
                    had_error = 1;
                } else if (copy_string(out->session_type,
                                       sizeof(out->session_type), value) < 0) {
                    had_error = 1;
                }
                ...
                if (parse_bounded_int(value, 0, MAX_SCREENSHOT_DELAY_MS,
                    ...
                    log_msg(LOG_ERROR,
                            "config:%d: screenshot_delay_ms must be 0..%d",
                ...
                if (parse_bounded_int(value, MIN_SCREENSHOT_TIMEOUT_MS,
                    ...
                    log_msg(LOG_ERROR,
                            "config:%d: screenshot_timeout_ms must be %d..%d",
    ...
    if (fclose(f) != 0) {
        log_msg(LOG_ERROR, "config: close failed for %s: %s", path, strerror(errno));
        had_error = 1;
    }
...
int config_resolve_group(const config_t *cfg, const char *app_name,
    ...
    for (int i = 0; i < cfg->app_count; i++) {
        if (strcmp(cfg->apps[i].app_name, app_name) == 0) {
            if (copy_string(out_group, out_group_sz,
                            cfg->apps[i].group_name) < 0)
                return -1;
            return 1;
        }
    }
